...
| Path | Description |
| --- | --- |
| `rtl/` | Synthesizable SystemVerilog: `riscv_core.v`, `alu.v`, pipeline registers, hazard unit, decoder, regfile, etc. |
| `tb/` | Self-checking testbench (`tb_riscv_core.v`) with behavioral instruction/data memories and PASS/FAIL scoreboard. |
| `mem/` | `gen_program.py` (Fibonacci workload generator) plus the generated `program.hex` consumed by `$readmemh`. |
| `build/` | Auto-created output directory for `iverilog`; safe to delete. |
| `cpu_wave.vcd` | Waveform dump generated by `make run`; open in GTKWave. |
| `ARCHITECTURE_SOP.md`, `PIPELINE.md`, `WAVEFORM.md`, `GLOSSARY.md` | Reference docs on design intent, hazards, waveform tips, and terminology. |
